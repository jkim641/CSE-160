...
var FSHADER_SOURCE =
  ...
  '  vec3 specular = u_lightColor * pow(max(dot(E,R), 0.0), 10.0)*0.5;\n' +
  ...
  '  vec3 spotDiffuse = vec3(gl_FragColor) * vec3(1.0,1.0,1.0) * spotEffect * spotNDotL;\n' +
...
let g_spotLightPos = [0, 2.0, -1.0];

let g_spotLightDir = [0, -1, 0.5];
...
{
  //canvas
  canvas = document.getElementById('asmt4-lighting');
...
function renderScene()
{
  ...
  testCube.color = white
  testCube.matrix.translate(-2.0, 0.0, -0.5);
  ...
  spotLightCube.matrix.translate(
    g_spotLightPos[0],
    g_spotLightPos[1],
    g_spotLightPos[2]
  );
  ...
  if (g_objModel) {
    ...
    g_objModel.color = [1.0, 0.84, 0.2, 1.0];

    g_objModel.matrix.setTranslate(0, 1.2, 0);

    g_objModel.matrix.rotate(180, 0, 1, 0);

    g_objModel.matrix.scale(0.06, 0.06, 0.06);
  ...
  platform.matrix.scale(3.0, 0.54, 3.0);       
  platform.render();
...
function addActionsForHtmlUI()
{
  ...
  document.getElementById('lightSlideX').addEventListener('mousemove', function() {
    ...
    g_lightPos[0] = -this.value / 20;

  });

  document.getElementById('lightSlideY').addEventListener('mousemove', function() {

    g_lightPos[1] = this.value / 20;

  });

  document.getElementById('lightSlideZ').addEventListener('mousemove', function() {

    g_lightPos[2] = this.value / 20;

  });

  document.getElementById('lightRed').addEventListener('mousemove', function() {

    g_lightColor[0] = this.value / 100;

  });

  document.getElementById('lightGreen').addEventListener('mousemove', function() {

    g_lightColor[1] = this.value / 100;

  });

  document.getElementById('lightBlue').addEventListener('mousemove', function() {
...
function tick()
{
  ...
  if (g_keys[87]) g_camera.moveForward();    
  if (g_keys[83]) g_camera.moveBackwards();   
  if (g_keys[65]) g_camera.moveLeft();       
  if (g_keys[68]) g_camera.moveRight();       
  if (g_keys[81]) g_camera.panLeft();        
  if (g_keys[69]) g_camera.panRight();       
...
function updateAnimationAngles()
{
  if (g_yellowAnimation) {
    ...
    g_lightPos[0] = Math.cos(g_seconds) * 4.0;
    g_lightPos[1] = Math.sin(g_seconds) * 4.0 + 0.5;
